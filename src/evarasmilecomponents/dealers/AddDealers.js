...
function AddDealers({ open, onClose, onDealerAdded }) {
    ...
    const handleSubmit = async () => {
        try {
            const response = await axios.post('https://namami-infotech.com/EvaraBackend/SalesSmile/src/dealer/add_dealers.php', {
                dealerName,
                address,
                contactInfo,
                latLong,
    ...
    return (
        <Dialog open={open} onClose={onClose}>
            <DialogTitle style={{ backgroundColor: "teal", color: "white" }}>Add Dealer</DialogTitle>
            <DialogContent>
                <TextField
                    label="Dealer Name"
                    value={dealerName}
