...
function PolicyList() {
    ...
    const fetchPolicies = async () => {
        try {
            const response = await axios.get('https://namami-infotech.com/EvaraBackend/SalesSmile/src/policy/view_policy.php');
    ...
    const handleTogglePolicyStatus = async (policyId, action) => {
        try {
            const response = await axios.post('https://namami-infotech.com/EvaraBackend/SalesSmile/src/policy/disable_policy.php', { PolicyId: policyId, action });
            if (response.data.success) {
    ...
    return (
        <div>
            {user && user.role === 'HR' ? <Button variant="contained" color="primary" onClick={handleOpenDialog} sx={{ mb: 2 }} style={{ backgroundColor: "teal" }}>
                Add Policy
            </Button> : null}
